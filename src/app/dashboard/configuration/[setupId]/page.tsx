...
const Configuration = () => {
	...
	const fetchSetup = useCallback(async () => {
		try {
			...
			const response = await fetch(`${apiUrl}scheduled_playlists/${setupId}`, {
				headers: {
	...
	const handleSavePlaylist = async (updatedPlaylist: PlaylistData) => {
		try {
			...
			const url = setupId === 'new' ? `${apiUrl}scheduled_playlists` : `${apiUrl}scheduled_playlists/${setupId}`;
			...
			<main className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8 mt-16">
				<div className="px-4 py-6 sm:px-0">
					<div className="space-y-8">
						...
						<div>
							<Label htmlFor="setup-name" className="text-lg font-medium mb-2 block">
								Setup Name
							</Label>
							<Input
								id="setup-name"
						...
						<div>
							<Label htmlFor="device-select" className="text-lg font-medium mb-2 block">
								Connected Devices
							</Label>
							<Select value={selectedDevice}>
								<SelectTrigger id="device-select" className="border-2 shadow-md bg-white text-black w-2/4 placeholder-gray-500">
